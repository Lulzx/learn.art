...
1. Dense tensors store arbitrary-rank floating-point values with explicit device and dtype identity. Read-only slice views borrow source ownership, while device-resident results and views acquire an owned row-major host block only at a materialization boundary.
